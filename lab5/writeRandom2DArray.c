...
void read2DArray(int * rows, int * columns, double *** array2DRead, 
  ...
  // read rows and columns from bin file
  fread(&(*rows), sizeof(int), 1, inputFilePtr);
  fread(&(*columns), sizeof(int), 1, inputFilePtr);
  ...
  for (r=0; r < *rows; r++) {
    //reads a whole row of the array
    fread(local2DArray[r], sizeof(double), *columns, inputFilePtr);
  } // end for
 
  *array2DRead = local2DArray;
