...
int main(int argc, char* argv[]) {
  int myID, value, numProcs, i, p, sum=0;
  float * myArray;
  float * localArray;
  ...
  int * displacements;
  int length;
  int length_per_process,recvBufNum;
  ...
  displacements = (int*)malloc(sizeof(int)*numProcs);

  for (i = 0; i < numProcs; i++) {
    counts[i] = length_per_process;
    displacements[i] = sum;
    sum += counts[i];
  }
  counts[numProcs-1]+=length%numProcs;

  localArray = (float *) malloc(length_per_process*sizeof(float));
  printf("myID %d\n", myID);
  recvBufNum=counts[myID];
  // distributes chucks of data to all processes
  MPI_Scatterv( myArray, counts, displacements, MPI_FLOAT, localArray,
	              recvBufNum, MPI_FLOAT, RootProcess,  MPI_COMM_WORLD );
