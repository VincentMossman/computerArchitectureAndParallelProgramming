 ...
 *
 * Compile:  gcc -O3 -Wall -o tsp_iter1 tsp_iter1.c
 *           Needs timer.h
...
int  promising(tour_t tour, city_t city);
...
#  endif
   ...
   while (!EMPTY(stack)) {
      ...
      else if (promising(curr_tour, city)) { // Only check cost (visited checked
 ...
 */
int Feasible(tour_t tour, city_t city) {
   printf("Checking to see if city %d is feasible and ", city);
   city_t last_city = Last_city(tour);

   if (Tour_cost(tour) + Cost(last_city,city) < Tour_cost(best_tour)) {
      printf("it is FEASIBLE\n");
      return TRUE; }
   else {
      printf("it is NOT FEASIBLE\n");
      return FALSE; }
}  /* Feasible */
...
int promising(tour_t tour, city_t city) {
  ...
  int i, test;
  int minEdge;
  int pathLength = Tour_cost(tour);
  int nextVertex = city;

  // update the partial path to reflect the child
  pathLength += Cost(Last_city(tour),city);
  bound = pathLength;

  // find every minEdge
  for (test = 1; test < n; test++) {
   if (!Visited(tour,test)) {
     minEdge = INFINITY;
     for (i = 0; i < n; i++) {
       if (i==0 || (i!=nextVertex && test != i && Cost(test,i) < minEdge)) {
         minEdge = Cost(test,i);
       } // end if
     } // end for (i
     bound += minEdge;
  ...
  // putting the not visited condition down here seemed to work for some reason
  if (!Visited(tour, city) && bound < Tour_cost(best_tour)) {
    //printf("PROMISING bound: %d Tour_cost(best_tour): %d\n",bound,Tour_cost(best_tour));
    return TRUE;
  } else {
    //printf("NOT PROMISING bound: %d Tour_cost(best_tour): %d\n",bound,Tour_cost(best_tour));
    return FALSE;
  } // end if
} // end promising
